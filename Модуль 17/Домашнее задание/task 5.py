...
inrex_list = [i_num for i_num in range(len(word)) if word[i_num] == 'h']
print(word[:min(inrex_list)+ 1] + word[max(inrex_list)-1: min(inrex_list):-1] + word[max(inrex_list):])
print('Развёрнутая последовательность между первым и последним h:', word[max(inrex_list)-1: min(inrex_list):-1])