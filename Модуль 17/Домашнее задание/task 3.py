...
(то есть две команды) по 20 участников в каждом. В этих списках хранятся очки каждого участника
(это вещественные числа с двумя знаками после точки, например 4.03). Участник одной команды соревнуется с
участником другой команды под таким же номером. То есть первый соревнуется с первым, второй — со вторым и так далее.
...
group_1 = [round(random.uniform(5, 10), 2) for _ in range(20)]
group_2 = [round(random.uniform(5, 10), 2) for _ in range(20)]
result = [(group_1[i_num] if group_1[i_num] > group_2[i_num] else group_2[i_num]) for i_num in range(20)]
print('Первая команда:', group_1)
print('Вторая команда:', group_2)
