...
Ваня работает middle-разработчиком на Python в IT-компании. Один кандидат на позицию junior-разработчика
прислал ему код тестового задания.

В задании был словарь из трёх студентов. Необходимо:
Вывести на экран список пар «ID студента — возраст».
Написать функцию, которая принимает в качестве аргумента словарь и возвращает два значения: полный список
интересов всех студентов и общую длину всех фамилий студентов.
...
(только если очень захочется). Убедитесь, что программа верно работает Проверки на существование записей в
...
def func(dict):
    ...
    for i in dict:
        lst += (dict[i]['interests'])
        string_len += len(dict[i]['surname'])

    return lst, string_len

pairs = [(i, students[i]['age']) for i in students]
my_lst = func(students)

print('Список пар "ID студента — возраст": {age}, '
      '\nПолный список интересов всех студентов: {interests}, '
      '\nОбщая длина всех фамилий студентов: {count}'.format(
        age=pairs,
        interests=my_lst[0],
        count=my_lst[1]
    ))