...
if task == 1:
    ...
    number = int(input('Какой факториал ищем: '))
    print('Факториал числа {number} равняется {result}'.format(
        number=number,
    ...
    def func(search, site):
        if search in site:
            return site[search]
        for place in site.values():
            if isinstance(place, dict):
                result = func(search, place)
                if result:
                    break
        else:
            result = None
    ...
    result = func(search, site)
    if result:
        print('Результат: {res}'.format(res=result))
    else:
